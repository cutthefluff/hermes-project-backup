...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if not service_available:
            ...
            # Manual restart: stop only this profile's gateway, then relaunch
            # detached.  Jacob's Docker image runs the gateway as a background
            # child, not a real systemd/s6 service; foreground run_gateway()
            # makes the replacement depend on the shell/SSH session that ran
            # the command.
            from gateway.status import get_running_pid
            from hermes_cli.profiles import get_active_profile_name

            old_pid = get_running_pid(cleanup_stale=False) or 0
            if stop_profile_gateway():
                print("✓ Stopped gateway for this profile")
