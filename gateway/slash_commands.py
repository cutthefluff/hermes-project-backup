...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        # Jacob's Docker image starts the gateway as a background child from
        # /entrypoint.sh while PID 1 stays alive as the dashboard.  Treating
        # every container as service-managed makes /restart exit with code 75
        # and nothing respawns it.  Only use the service/container restart path
        # when the gateway itself is PID 1 or systemd-managed; otherwise use
        # Hermes's detached restart helper so Telegram /restart works without
        # SSHing in to start a new foreground gateway.
        if _under_service or (_in_container and os.getpid() == 1):
